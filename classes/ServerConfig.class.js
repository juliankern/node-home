module.exports = utils => class ServerConfig {
    validConfiguration(config, format) {
        ...
        function parse(config, format, parentkey) {
            ...
            if (+config.room === 0) {
                errors.push({
                    fields: ['room'], message: 'You need to select a room or create a new one.' });
                return false;
            }
            ...
            for (let k in format) {
                ...
                if (!format[k].description || format[k].description.length === 0) {
                    errors.push({
                        fields: [parentkey ? parentkey + k : k],
                        message: `Field "${format[k].description}" lacks a description.`,
                    });
                    break;
                }
                ...
                if (
                    (format[k].type === 'number' && format[k].required
                        && config[parentkey ? parentkey + k : k] === '') ||
                    (format[k].type === 'array' && format[k].required
                        && !config[`${(parentkey ? parentkey + k : k)}.0`]) ||
                    (format[k].type !== 'number' && format[k].type !== 'array'
                        && format[k].required && !config[parentkey ? parentkey + k : k])
                ) {
                    errors.push({
                        fields: [parentkey ? parentkey + k : k],
                        message: `The required field "${format[k].description}" isn't set!`,
                    });
                    break;
                }
                ...
                if (format[k].type === 'string' && typeof config[parentkey ? parentkey + k : k] !== 'string') {
                    errors.push({
                        fields: [parentkey ? parentkey + k : k],
                        message: `The field "${format[k].description}" has the wrong type!`,
                    });
                    break;
                }
                ...
                if (format[k].type === 'number' && typeof +config[parentkey ? parentkey + k : k] !== 'number') {
                    errors.push({
                        fields: [parentkey ? parentkey + k : k],
                        message: `The field "${format[k].description}" has the wrong type! (2)`,
                    });
                    break;
                }
                ...
                if (format[k].type === 'select' && !(format[k].values && format[k].values.length > 0)) {
                    errors.push({
                        fields: [parentkey ? parentkey + k : k],
                        message: `The field "${format[k].description}" is not properly configured. (1)`,
                    });
                    break;
                }
                ...
                if (format[k].type === 'array' && !format[k].length) {
                    errors.push({
                        fields: [parentkey ? parentkey + k : k],
                        message: `The field "${format[k].description}" is not properly configured. (2)`,
                    });
                    break;
                }

                // if is an array and has configured values
                if (format[k].type === 'array' &&
                    (typeof config[parentkey ? parentkey + k : k] === 'object' &&
                        config[parentkey ? parentkey + k : k].length === format[k].length)
                ) {
                    errors.push({
                        fields: [parentkey ? parentkey + k : k],
                        message: `The field "${format[k].description}" is not properly configured. (3)`,
                    });
                    break;
                }
            }
